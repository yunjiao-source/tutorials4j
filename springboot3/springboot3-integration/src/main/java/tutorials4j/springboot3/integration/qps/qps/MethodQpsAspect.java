...
public class MethodQpsAspect {
  ...
  public Object trackQps(ProceedingJoinPoint joinPoint, MethodQps methodQps) throws Throwable {
    ...
    // 获取或创建计数器
    Counter counter =
        methodCounters.computeIfAbsent(
            methodName,
            k ->
    ...
    // 获取或创建计时器
    Timer timer =
        methodTimers.computeIfAbsent(
            methodName,
            k ->
                Timer.builder("method_call_duration")
                    .tag("method", k)
    ...
    try {
      ...
      // 记录到最近100次调用时间
      CallTimeRecorder recorder =
          methodCallTimes.computeIfAbsent(methodName, k -> new CallTimeRecorder());
