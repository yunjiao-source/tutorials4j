...
- **模板加载与编译**：从 classpath 下的 `jasperreports/user.jrxml` 加载报表模板，并编译为 `JasperReport` 对象。
...
- **端点**：`GET /users/export/{format}`，其中 `{format}` 为 `pdf`、`xml` 或 `html`。
- **模拟数据**：使用 `Faker` 库生成 10 条随机用户数据（身份证、姓名、年龄、邮箱、地址）。
- **报表生成**：调用 `ReportGenerator.generate(users, format)` 获取报表字节内容。
- **响应构建**：
    - `Content-Type`：`application/octet-stream`
    - `Content-Disposition`：`attachment; filename="user-report.{format}"`，强制浏览器下载。
    - 返回 `ByteArrayResource` 包装的字节数组。

### 整体流程
1. 客户端请求 `/users/export/pdf`（或 xml/html）。
2. 服务端生成随机用户列表。
3. 根据模板和数据生成指定格式的报表。
4. 以附件形式返回报表文件。

### 注意事项
- 报表模板文件 `jasperreports/user.jrxml` 必须存在于 classpath 中，且模板内字段需与 `User` 类的属性名匹配（如 `id`、`name` 等）。
