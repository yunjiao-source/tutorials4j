...
public class ConnectionPoolMonitor {
  ...
  private PoolStatistics collectMetrics(
      HikariPoolMXBean poolMXBean, HikariConfigMXBean configMXBean) {
  ...
  private void cleanupOldMetrics() {
    long cutoff =
        System.currentTimeMillis() - properties.getMonitor().getMetricsRetentionTime().toMillis();

    while (!metricsHistory.isEmpty() && metricsHistory.peek().getTimestamp() < cutoff) {
      metricsHistory.poll();
    }
  }
