...
| `UserController` | 演示四种使用场景：<br> - `GET /mybatisUser`：直接使用 `@UserFormat` 标注的方法参数。<br> - `POST /mybatisUser`：与 `@RequestParam` 结合，接收表单参数。<br> - `GET /mybatisUser-wrapper`：`UserWrapper` 中的 `mybatisUser` 字段带有 `@UserFormat`，自动转换。<br> - `POST /mybatisUser-wrapper`：使用 `@ModelAttribute` 绑定，同样支持字段级注解。 |
| `UserWrapper` | 包装类，包含带 `@UserFormat` 的 `mybatisUser` 字段和一个 `age` 字段。 |

### 工作流程

1. 客户端发送请求，其中 `mybatisUser` 参数（或表单字段）值为 `"1,张三"`。
2. Spring MVC 发现该参数或字段带有 `@UserFormat` 注解，且类型为 `User`。
3. 调用注册的 `StringToUserFormatter.getParser()` 返回的解析器。
4. 解析器执行：`text.split(",")` → `mybatisUser.setId(1L)`、`mybatisUser.setName("张三")`。
