...
`SignInService` 和 `SignInTemplate` 构造时接收一个 `SignInResultHandler` 实例，每次签到完成后会自动调用其 `handle(response)` 方法。开发者可以实现该接口来记录日志、发放奖励、发送消息等。代码中提供了一个简单的日志实现 `LoggingSignInResultHandler`。
...
public void demo() {
    ...
    SignInResult response = template.signIn("user123", today);
    System.out.println("签到成功：" + response.signedIn());
    System.out.println("连续签到：" + response.continuousDays() + "天");
