...
public class TreeQueryTest {
  ...
  void testCircularReference() {
    ...
    List<Node> cyclicNodes = Arrays.asList(new Node(10, 11), new Node(11, 10));
    Predicate<Integer> rootPredicate = id -> false; // 无根节点
    TreeQuery<Integer, Node> cyclicQuery =
        new TreeQuery<>(Node::getId, Node::getParentId, rootPredicate);
  ...
  void testBuilder() {
    ...
    TreeQuery<Integer, Node> builtQuery =
  ...
  void testBuilderWithoutNodes() {
    TreeQuery<Integer, Node> emptyQuery =
        new TreeQuery.TreeQueryBuilder<Integer, Node>()
            .withIdGetter(Node::getId)
            .withParentIdGetter(Node::getParentId)
            .withRootId(ROOT_ID)
  ...
  void testMultipleRoots() {
    // 构建多棵树: 根节点1和根节点8
    List<Node> multiRootNodes =
        Arrays.asList(new Node(1, null), new Node(2, 1), new Node(8, null), new Node(9, 8));
    Predicate<Integer> rootPredicate = id -> id == 1 || id == 8;
    TreeQuery<Integer, Node> multiQuery =
        new TreeQuery<>(Node::getId, Node::getParentId, rootPredicate);
