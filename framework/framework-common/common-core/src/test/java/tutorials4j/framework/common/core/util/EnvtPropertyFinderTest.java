...
/**
 * {@link EnvtPropertyFinder} 单元测试
 *
 * @author Yun Jiao
 */
public class EnvtPropertyFinderTest {
    ...
    void testGetPropertyFromEnvironment() {
        runner.run(context -> {
            ...
            // 测试 String 属性
            String stringValue = EnvtPropertyFinder.getProperty(env, "test.string", String.class);
            assertThat(stringValue).isEqualTo("hello");

            // 测试带默认值的 String
            String missingWithDefault = EnvtPropertyFinder.getProperty(env, "missing.key", String.class, "default");
            assertThat(missingWithDefault).isEqualTo("default");

            // 测试 Boolean
            Boolean boolValue = EnvtPropertyFinder.getProperty(env, "test.bool", Boolean.class);
    ...
    void testGetStringPropertyViaConditionContext() {
        ...
        runner.run(context -> {
            ...
            String value = EnvtPropertyFinder.getStringProperty(context, "test.string");
            assertThat(value).isEqualTo("hello");

            Boolean flag = EnvtPropertyFinder.getBoolProperty(context, "test.bool", false);
