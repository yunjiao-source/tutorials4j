 ...
 * <p>类似于 Spring 的 AbstractRoutingDataSource，此类允许在运行时动态选择底层缓存管理器，
 * 适用于多租户、环境隔离等场景。</p>
 *
...
public abstract class AbstractRoutingCacheManager<T extends CacheManager> implements CacheManager {
     ...
     * <p>内部通过 {@link #determineTargetDataSource()} 确定当前目标缓存管理器，
     * 然后委托给该管理器的 {@code getCache} 方法。</p>
     *
    ...
    /**
     * 确定当前要使用的目标缓存管理器。
     * <p>首先调用 {@link #determineCurrentLookupKey()} 获取查找键，
     * 然后从内部映射中获取对应的管理器；如果不存在则通过
     * {@link #createCacheManager(Object)} 动态创建并缓存。</p>
     *
     ...
     * <p>实现类通常从 {@link ThreadLocal} 或类似上下文中获取租户 ID、环境标识等。</p>
     *
     * @return 查找键
     */
    @NonNull
