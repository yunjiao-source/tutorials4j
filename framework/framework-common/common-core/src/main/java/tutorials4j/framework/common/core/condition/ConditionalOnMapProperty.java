 ...
 * <p>该注解基于 Spring Boot 的 {@link Conditional} 机制，通过 {@link OnCollectionMapCondition}
 * 实现具体匹配逻辑。</p>
 *
 * <p><b>使用示例：</b>
 * <pre>{@code
...
public @interface ConditionalOnMapProperty {
    ...
    /**
     * 配置前缀，例如 "my.map"。
     * <p>与 {@link #name()} 拼接形成完整的配置键，如 "my.map.settings"。</p>
     *
     * @return 配置前缀，默认为空字符串
     */
     ...
     * 是否要求 Map 为空。
     * <ul>
     *   <li>{@code true}：Map 为空时条件匹配</li>
     *   <li>{@code false}：Map 非空时条件匹配</li>
     * </ul>
     *
