 ...
 * XSS 攻击防御工具类。
 * <p>
 * 基于 OWASP AntiSamy 实现，通过策略文件（antisamy-anythinggoes.xml）对用户输入的 HTML 或文本进行过滤，
 * 移除恶意脚本及危险的 HTML 标签/属性。同时处理特殊字符转义及乱码问题。
 * </p>
 * <p>
 * 该类为单例模式，内部持有 {@link AntiSamy} 实例及清洗过程中产生的特殊字符占位符（&nbsp;、双引号等）。
 * </p>
 *
 * @author Yun Jiao
 * @see AntiSamy
...
public class XssUtils {
     ...
     * <p>
     * 尝试从 classpath 加载策略文件 {@code antisamy/antisamy-anythinggoes.xml}，
     * 若失败则使用默认策略。同时预先清洗 "&nbsp;" 和 "\"" 字符串，用于后续结果修正。
     * </p>
     */
    private XssUtils() {
     ...
     * 执行步骤：
     * <ol>
     *   <li>对输入字符串中的 HTML 实体（如 &amp;lt; ）进行反转义，因为 AntiSamy 扫描时会再次转义；</li>
     *   <li>调用 AntiSamy 清洗；</li>
     *   <li>将清洗结果中的乱码（&nbsp; 转换后的异常字符）替换为空字符串；</li>
     *   <li>将双引号的乱码替换为原始双引号；</li>
     *   <li>移除换行符。</li>
     * </ol>
     * </p>
     *
     ...
     * 创建 AntiSamy 策略对象。
     * <p>
     * 从 classpath 中的 {@code antisamy/antisamy-anythinggoes.xml} 加载策略文件。
     * </p>
     *
