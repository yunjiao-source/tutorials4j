 ...
 * 通用 Bean 创建器函数式接口。
 * <p>
 * 定义了一个获取实例的核心方法，并提供了创建新实例及获取 Bean 类型的默认实现。
 * 子类可以根据需要覆盖 {@link #newInstance()} 和 {@link #getBeanClass()} 方法。
 * </p>
 *
 * @param <T> 创建的 Bean 类型
 * @author Yun Jiao
...
public interface BeanCreator<T> {
     ...
     * <p>默认实现抛出 {@link UnsupportedOperationException}，子类应重写此方法以提供实际创建逻辑。</p>
     *
     * @return 新创建的 Bean 实例
     ...
     * <p>默认实现抛出 {@link UnsupportedOperationException}，子类应重写此方法以提供实际类型信息。</p>
     *
