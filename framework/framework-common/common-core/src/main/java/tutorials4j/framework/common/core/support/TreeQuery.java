...
public class TreeQuery<ID, T> {
   ...
   */
  public TreeQuery(
      Function<T, ID> idGetter, Function<T, ID> parentIdGetter, Predicate<ID> rootIdPredicate) {
  ...
  public void removeNode(ID nodeId) {
    ...
    if (parentToChildrenMap.containsKey(parentId)) {
      parentToChildrenMap.get(parentId).removeIf(n -> idGetter.apply(n).equals(nodeId));
    }
