...
/**
 * 使用 Redis 自动续期锁保护的任务执行器接口。
 *
 * <p>任务执行前通过 {@link RedisLockService} 的自动续期锁模式加锁，避免任务因锁过期被并发重复执行； 加锁成功后调用 {@link #doRun(Map)}
 * 执行实际业务逻辑，异常统一交由 {@link #handleException(Exception)} 处理。
 *
 * @author Yun Jiao
 */
