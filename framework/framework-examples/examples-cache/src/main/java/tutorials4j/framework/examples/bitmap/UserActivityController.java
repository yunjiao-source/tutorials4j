...
public class UserActivityController {
  ...
  /** 用户签到 POST /api/bitmap/sign?userId=1001&date=2026-05-13 */
  @PostMapping("/sign")
  ...
  /** 检查某天是否签到 GET /api/bitmap/sign/check?userId=1001&date=2026-05-13 */
  @GetMapping("/sign/check")
  public Result<Boolean> checkSign(
      @RequestParam("userId") Long userId,
      @RequestParam("date") @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate date) {
    String key = SIGN_PREFIX + userId + ":" + date.format(DateTimeFormatter.ofPattern("yyyyMM"));
  ...
  /** 统计用户当月累计签到天数 GET /api/bitmap/sign/count?userId=1001&yearMonth=202605 */
  @GetMapping("/sign/count")
  public Result<Long> countSignInMonth(
      @RequestParam("userId") Long userId,
  ...
  /** 记录当日活跃用户（用户登录时调用） POST /api/bitmap/active/record?userId=1001 */
  ...
  /** 获取某日活跃用户数（日活） GET /api/bitmap/active/dau?date=2026-05-13 */
  ...
  /** 获取连续 N 天活跃的用户数（AND 运算） GET /api/bitmap/active/consecutive?startDate=2026-05-10&days=3 */
  @GetMapping("/active/consecutive")
  public Result<Long> getConsecutiveActiveUsers(
  ...
  /** 获取一周内（周一~周日）有过活跃的用户数（OR 运算） GET /api/bitmap/active/weekly?anyDate=2026-05-13 */
  @GetMapping("/active/weekly")
  public Result<Long> getWeeklyActiveUsers(
      @RequestParam("anyDate") @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate anyDate) {
