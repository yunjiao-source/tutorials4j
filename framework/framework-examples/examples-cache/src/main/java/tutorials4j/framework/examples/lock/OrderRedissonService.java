...
public class OrderRedissonService {
  ...
  public void blockNonExpireTime(String orderId) {
    log.info("blockNonExpireTime - {}", Thread.currentThread().getName());
    int time = sleep();
    log.info("blockNonExpireTime - {}, 时长：{}", Thread.currentThread().getName(), time);
  }

  @RedissonBlockLockable(key = "#root.args[0]", prefix = "order:", expireTime = 3000)
  public void blockExpireTime(String orderId) {
    log.info("blockExpireTime - {}", Thread.currentThread().getName());
    int time = sleep();
    log.info("blockExpireTime - {}, 时长：{}", Thread.currentThread().getName(), time);
  }

  @RedissonReentrantLockable(key = "#root.args[0]", prefix = "order:")
  public void reentrantNonExpireTime(String orderId) {
    log.info("reentrantNonExpireTime - {}", Thread.currentThread().getName());
    int time = sleep();
    log.info("reentrantNonExpireTime - {}, 时长：{}", Thread.currentThread().getName(), time);
  }
