...
public class MapReduceProcessorDemo implements MapReduceProcessor {
  ...
  public ProcessResult process(TaskContext context) throws Exception {
    ...
    // PowerJob 提供的日志 API，可支持在控制台指定多种日志模式（在线查看 / 本地打印）。最佳实践：全部使用 OmsLogger 打印日志，开发阶段控制台配置为
    // 在线日志方便开发；上线后调整为本地日志，与直接使用 SLF4J 无异
    OmsLogger omsLogger = context.getOmsLogger();

    // 是否为根任务，一般根任务进行任务的分发
    boolean isRootTask = isRootTask();
    // Task 名称，除了 MAP 任务其他 taskName 均由开发者自己创建，某种意义上也可以按参数理解（比如多层 MAP 的情况下，taskName 可以命名为，Map_Level1,
    // Map_Level2，最终按 taskName 判断层级进不同的执行分支）
    ...
    omsLogger.info(
        "[MapReduceDemo] [startExecuteNewTask] jobId:{}, instanceId:{}, taskId:{}, taskName: {}, RetryTimes: {}, isRootTask:{}, jobParams:{}, instanceParams:{}",
        context.getJobId(),
        context.getInstanceId(),
        context.getTaskId(),
        taskName,
        context.getCurrentRetryTimes(),
        isRootTask,
        jobParamsStr,
        instanceParamsStr);
    ...
    final JSONObject params =
        Optional.ofNullable(finalParams).map(JSONObject::parseObject).orElse(new JSONObject());
    ...
    if (isRootTask) {
      ...
      // 每个子任务携带多少个文件ID（此参数越大，每个子任务就“越大”，如果失败的重试成本就越高。参数越小，每个子任务就越轻，当相应的分片数量会提升，会让 PowerJob
      // 计算开销增大，建议按业务需求合理调配）
      ...
      for (long i = 0; i < num; i++) {
        ...
        if (ids.size() >= batchSize) {
          ...
          SubTask subTask =
              new SubTask(ThreadLocalRandom.current().nextLong(), Lists.newArrayList(ids), "extra");
          ids.clear();
    ...
    // 如果是简单的二层结构（ROOT - SubTASK），此处一定是子 Task，无需再次判断。否则可使用 TaskContext#taskName 字符串匹配 或
    // TaskContext#SubTask 对象内自定义参数匹配，进入目标执行分支

    // 获取前置节点 map 传递过来的参数，进行业务处理
    SubTask subTask = (SubTask) context.getSubTask();
    log.info(
        "[MapReduceDemo] [SubTask] taskId:{}, taskName: {}, subTask: {}",
        context.getTaskId(),
        taskName,
  ...
  public ProcessResult reduce(TaskContext context, List<TaskResult> taskResults) {
    ...
    // 所有 Task 执行结束后，reduce 将会被执行，taskResults 保存了所有子任务的执行结果。（注意 reduce
    // 由于保存了所有子任务的执行结果，在子任务规模巨大时对内存有极大开销，超大型计算任务慎用或使用流式 reduce（开发中））
    ...
    String resultMsg =
        String.format(
            "succeedTaskNum:%d,failedTaskNum:%d,successRate:%f",
            successCnt.get(), failedCnt.get(), successRate);
  ...
  /**
   * 自定义的子任务，按自己的业务需求定义即可 注意：代表子任务参数的类：一定要有无参构造方法！一定要有无参构造方法！一定要有无参构造方法！ 最好把 GET / SET
   * 方法也加上，减少序列化问题的概率
   */
  @Data
  @AllArgsConstructor
