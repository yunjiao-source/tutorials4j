 ...
 *
 * <p>该创建器负责创建和缓存 {@link TenantCaffeineCacheManager} 实例。 它内部委托给普通的 {@link
 * CaffeineCacheManagerCreator} 来完成底层 Caffeine 缓存管理器的构建， 然后用租户隔离的包装器进行装饰。
 *
 * @author Yun Jiao
...
public class TenantCaffeineCacheManagerCreator
    implements CacheManagerCreator<TenantCaffeineCacheManager> {
