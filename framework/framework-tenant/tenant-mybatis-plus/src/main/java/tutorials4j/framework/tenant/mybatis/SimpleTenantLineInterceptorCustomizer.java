 ...
 * 默认租户拦截器自定义器。
 *
 * <p>向 MyBatis Plus 拦截器中添加 {@link TenantLineInnerInterceptor}， 使用 {@link DefaultTenantLineHandler}
 * 处理租户逻辑，从 {@link tutorials4j.framework.common.core.TenantContextHolder} 中获取当前租户 ID， 并忽略配置中指定的表。
 *
 * @author Yun Jiao
...
public class SimpleTenantLineInterceptorCustomizer implements MybatisPlusInterceptorCustomizer {
  ...
  public void custom(MybatisPlusInterceptor interceptor) {
    TenantLineHandler tenantLineHandler =
        new DefaultTenantLineHandler(Set.of(properties.getMybatisPlus().getIgnoreTable()));
