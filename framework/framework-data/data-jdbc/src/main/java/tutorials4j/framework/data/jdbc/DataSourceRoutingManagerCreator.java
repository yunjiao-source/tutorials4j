 ...
 *
 * <p>根据默认数据源的实际类型（HikariCP、DBCP2、Druid）自动选择并实例化对应的 {@link DataSourceRoutingManager} 实现类，并执行初始化。
 *
 * <p>该类采用双重检查锁定的懒加载模式，确保线程安全且仅创建一次实例。
 *
 * @author Yun Jiao
...
public class DataSourceRoutingManagerCreator implements BeanCreator<DataSourceRoutingManager> {
   ...
   * 创建新的数据源路由管理器实例。
   *
   * <p>根据默认数据源的类名匹配对应的管理器实现：
   *
   * <ul>
   *   <li>com.zaxxer.hikari.HikariDataSource → {@link HikariMapDataSourceRoutingManager}
   *   <li>org.apache.commons.dbcp2.BasicDataSource → {@link Dbcp2MapDataSourceRoutingManager}
   *   <li>com.alibaba.druid.pool.DruidDataSource 或
   *       com.alibaba.druid.spring.boot3.autoconfigure.DruidDataSourceWrapper → {@link
   *       DruidMapDataSourceRoutingManager}
   * </ul>
   *
   * @return 新创建的路由管理器实例
  ...
  public DataSourceRoutingManager newInstance() {
    switch (dataSource.getClass().getSimpleName()) {
      case DefaultConsts.CLASS_HIKARI_DATA_SOURCE -> {
        HikariMapDataSourceRoutingManager dataSourceRoutingManager =
            new HikariMapDataSourceRoutingManager();

        dataSourceRoutingManager.init(dataSource);
        return dataSourceRoutingManager;
      }
      case DefaultConsts.CLASS_DBCP2_BASIC_DATA_SOURCE -> {
        Dbcp2MapDataSourceRoutingManager dataSourceRoutingManager =
            new Dbcp2MapDataSourceRoutingManager();

        dataSourceRoutingManager.init(dataSource);
        return dataSourceRoutingManager;
      }
      case DefaultConsts.CLASS_DRUID_DATA_SOURCE, DefaultConsts.CLASS_DRUID_DATA_SOURCE_WRAPPER -> {
        DruidMapDataSourceRoutingManager dataSourceRoutingManager =
            new DruidMapDataSourceRoutingManager();
