 ...
 *
 * <p>继承自 {@link AbstractDataSourceRoutingManager}，使用 {@code ConcurrentHashMap} 缓存已创建的数据源实例，
 * 当请求目标数据源时，若缓存中不存在则自动调用 {@link #createDataSource(String)} 创建并缓存。
 *
 * <p>初始化时会将默认数据源以 {@link DefaultConsts#DEFAULT_TENTANT_CODE} 为键放入缓存。
 *
 * @author Yun Jiao
...
public abstract class AbstractMapDataSourceRoutingManager extends AbstractDataSourceRoutingManager {
   ...
   * 使用默认数据源初始化路由管理器。
   *
   * <p>调用父类 {@link #init(DataSource, Map)} 并传入空路由配置， 同时将默认数据源存入缓存，键为 {@code
   * DefaultConsts.DEFAULT_TENTANT_CODE}。
   *
   ...
   * 根据名称确定要使用的目标数据源。
   *
   * <p>首先校验名称非空，然后从缓存中获取或创建对应的数据源实例。
   *
