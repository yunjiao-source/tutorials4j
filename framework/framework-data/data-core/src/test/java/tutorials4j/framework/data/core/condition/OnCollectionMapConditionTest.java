...
/**
 * 集成测试 {@link ConditionalOnMapProperty} 与 {@link OnMapPropertyCondition}
 * 使用 ApplicationContextRunner 模拟不同的配置环境
 */
/**
 * 集成测试 {@link ConditionalOnMapProperty} 与 {@link OnCollectionMapCondition}
 * 使用 ApplicationContextRunner 模拟不同的配置环境
 *
 * @author Yun Jiao
 */
class OnCollectionMapConditionTest {
    ...
    static class TestConfig {
        ...
        @Bean
        @ConditionalOnMapProperty(prefix = "my", name = "empty-map", isEmpty = true, matchIfMissing = false)
        String emptyMapBean() {
            return "emptyMapBean";
        }

        @Bean
        @ConditionalOnMapProperty(prefix = "missing", name = "prop", isEmpty = false, matchIfMissing = true)
        String missingWithMatchIfMissingTrueBean() {
            return "missingWithMatchIfMissingTrueBean";
        }

        @Bean
        @ConditionalOnMapProperty(prefix = "missing", name = "prop", isEmpty = false, matchIfMissing = false)
        String missingWithMatchIfMissingFalseBean() {
            return "missingWithMatchIfMissingFalseBean";
        }

        @Bean
        @ConditionalOnMapProperty(prefix = "alias", value = "custom-name", isEmpty = false, matchIfMissing = false)
        String aliasBean() {
            return "aliasBean";
        }
    ...
    void nonEmptyMapShouldMatchWhenIsEmptyFalse() {
        runner.withPropertyValues("my.map.key1=value1", "my.map.key2=value2")
                .run(context -> {
    ...
    void valueAliasShouldWorkWhenNameIsBlank() {
        runner.withPropertyValues("alias.custom-name.key=value")
                .run(context -> {
    ...
    void onlyPrefixShouldMatchWhenPrefixHasSubProperties() {
        runner.withPropertyValues("only-prefix.any-key=anyValue")
                .run(context -> {
    ...
    static class NameOverridesValueConfig {
        @Bean
        @ConditionalOnMapProperty(prefix = "test", name = "explicit-name", value = "ignored-value", isEmpty = false)
        String bean() {
            return "bean";
        }
    }
