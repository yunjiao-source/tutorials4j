 ...
 * 多级缓存模板抽象类。
 * <p>
 * 通过 {@link CacheManagerCreatorFactory#findMultiLevelCache(String)} 获取底层多级缓存实例（通常为一级本地缓存 + 二级远程缓存）。
...
public abstract class AbstractMultiLevelCacheTemplate<K, V> extends AbstractCacheTemplate<K, V> {
    ...
    protected void initCache() {
        cache = CacheManagerCreatorFactory.findMultiLevelCache(cacheName);
    }
}
