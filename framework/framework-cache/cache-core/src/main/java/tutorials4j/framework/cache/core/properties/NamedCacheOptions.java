 ...
 * 命名缓存的配置选项。
 * <p>
 * 包含各类缓存（如 Redis、Caffeine）的通用配置以及特定实现的相关配置。
 * 支持时间到期、空值缓存、统计开关等通用设置，以及 Redis 的键前缀、Caffeine 的初始容量等特有设置。
 * </p>
 *
 * @author Yun Jiao
...
public class NamedCacheOptions {
     ...
     * <p>
     * 对于时间到期、空值缓存开关、统计开关以及 Redis 键前缀，
     * 若当前对象中该属性为 {@code null}，则使用默认配置中的对应值。
     * 对于 Caffeine 配置中的初始容量和访问过期时间，直接使用默认配置中的值（覆盖当前值）。
     * </p>
     *
