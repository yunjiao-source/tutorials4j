 ...
 *
 * <p>用于为不同的缓存名称（cacheName）自动添加特定前缀，如租户隔离标识或自定义前缀。 同时提供了静态工具方法用于解析已添加前缀的 Key，以及常用的工厂方法生成特定策略实例。
 *
 * @author Yun Jiao
 * @see #tenant()
...
public interface RedisKeyPrefix {
   ...
   *
   * <p>值为 {@value}，取自 {@link SymbolConsts#COLON}。
   */
   ...
   *
   * <p>规则：若 Key 为空或空白，则原样返回；否则查找第一个冒号（{@value #SEPARATOR}）， 若找不到冒号则返回原 Key，否则返回冒号之后的所有字符。
   *
   * <p>示例：
   *
   * <pre>
   ...
   *
   * <p>生成的 Key 格式为：<code>{租户ID}:{cacheName}</code>。 租户 ID 通过 {@link TenantContextHolder#get()}
   * 动态获取。
   *
   * <p>典型用途：多租户系统中为不同租户的数据实现自然隔离。
   *
   * @return 基于租户的前缀策略函数
   ...
   *
   * <p>生成的 Key 格式取决于传入的 {@code prefix} 是否以分隔符结尾：
   *
   * <ul>
   *   <li>若 {@code prefix.endsWith(SEPARATOR)}，格式为：<code>{租户ID}:{prefix}{cacheName}</code>
   *   <li>否则格式为：<code>{租户ID}:{prefix}{SEPARATOR}{cacheName}</code>
   * </ul>
   *
   * <p>示例：
   *
