 ...
 *
 * <p>用于记录某个键（通常为业务标识）的访问次数，并提供次数上限控制。 缓存值为 {@link Long} 类型，表示当前已计数的次数。 核心方法 {@link
 * #counting(String)} 和 {@link #counting(String, int, boolean)} 在每次调用时自增计数， 当计数达到或超过最大允许次数时抛出 {@link
 * CounterOverflowException}。
 *
 * <p>默认采用普通字符串作为缓存键，可选择使用 MD5 对原始键进行摘要后再作为缓存键。 首次计数时会通过 {@link #valueGenerator(String)} 初始化缓存值为 1L。
 *
 * @author Yun Jiao
...
public abstract class AbstractCounterCacheTemplate
   ...
   *
   * <p>等价于调用 {@link #counting(String, int, boolean) counting(key, maxTimes, false)}。
   *
   * @param key 待计数的键
   ...
   * <ol>
   *   <li>校验键不能为 {@code null}；
   *   <li>若 {@code useMd5} 为 {@code true}，则对原始键进行 MD5 摘要作为实际缓存键；
   *   <li>从缓存中获取当前计数值，若不存在则初始化为 0；
   *   <li>若当前计数值为 0，则调用 {@link #create(Object)} 初始化缓存值为 1；
   *   <li>否则将计数值加 1 后更新缓存，并判断加 1 前的值是否已达到 {@code maxTimes - 1}（即本次操作后会超过上限），若是则抛出异常；
   *   <li>返回加 1 后的计数值。
   * </ol>
   *
   * @param key 待计数的键
