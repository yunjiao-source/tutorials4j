 ...
 *     允许指定等待获取锁的超时时间。</li>
 *     <li>若在等待时间内未能获取锁，会立即抛出 {@link DistributedLockException}，而不是无限阻塞。</li>
 *     <li>同样提供固定租约模式（需指定租约时间）和自动续期模式（看门狗）。</li>
 * </ul>
 * <p>
...
public class ReentrantRedissonLockService implements LockService {
    ...
    private final static Duration WAIT_TIME = Duration.ofSeconds(WAIT_SECONDS);
     ...
     * <p>
     * 使用 {@link RLock#tryLock(long, long, TimeUnit)} 方法。
     * 需同时指定等待获取锁的超时时间和锁的租约时间。
     */
    @RequiredArgsConstructor
    public static class FixedLease {
         ...
         */
        private RLock tryLock(String lockKey, Duration waitTime, Duration expireTime) throws InterruptedException {
            RLock lock = redissonClient.getLock(lockKey);
            boolean success = lock.tryLock(waitTime.toMillis(), expireTime.toMillis(), TimeUnit.MILLISECONDS);
            if (success) {
                return lock;
            }
         ...
         * <p>
         * 在指定的 {@code waitTime} 内尝试获取锁，若成功则执行任务，锁的持有时间上限为 {@code expireTime}。
         * 超时未获取到锁会抛出异常。
         *
         * @param lockKey    锁的键名
         * @param waitTime   等待获取锁的最大时间
         ...
         */
        public <T> T doInLock(String lockKey, Duration waitTime, Duration expireTime, Callable<T> task) {
            RLock lock = null;
            try {
     ...
     * <p>
     * 使用 {@link RLock#tryLock(long, TimeUnit)} 方法。
     * 只指定等待获取锁的超时时间，不指定租约时间，Redisson 会自动启用看门狗进行续期。
     */
    @RequiredArgsConstructor
    public static class AutoRenewal {
         ...
         * <p>
         * 在指定的 {@code waitTime} 内尝试获取锁，成功则执行任务，锁由看门狗自动续期。
         * 超时未获取到锁会抛出异常。
         *
         * @param lockKey  锁的键名
         * @param waitTime 等待获取锁的最大时间
