 ...
 * <p>
 * 两种模式均为阻塞式获取锁（即获取不到锁时会一直等待直到获得锁）。
 * <ul>
 *     <li><b>固定租约时间模式 ({@link FixedLease})</b>：使用 {@link RLock#lock(long, TimeUnit)} 方法，
 *     锁持有时间固定，到期自动释放，不会自动续期。</li>
 *     <li><b>自动续期模式 ({@link AutoRenewal})</b>：使用 {@link RLock#lock()} 方法，
 *     Redisson 会启动“看门狗”自动续期（默认每 10 秒续期一次，直到手动释放锁）。</li>
 * </ul>
 *
 * @author Yun Jiao
...
public class BlockRedissonLockService implements LockService {
     ...
     * <p>
     * 使用 {@link RLock#lock(long, TimeUnit)} 方法，锁在指定的 {@code expireTime} 后自动释放。
    ...
    public static class FixedLease {
         ...
         * <p>
         * 阻塞等待直到获取锁，持有锁的时间上限为 {@code expireTime}。
         * 任务执行完成后自动释放锁，若执行过程中抛出异常也会释放锁。
         *
         * @param lockKey    锁的键名
     ...
     * <p>
     * 使用 {@link RLock#lock()} 方法，无参调用时 Redisson 会自动启动看门狗线程，
     * 默认每 10 秒将锁的过期时间延长一次，确保业务执行期间锁不会自动释放。
     * <p>
     * <b>注意：</b> 该模式下必须手动调用 {@code unlock} 释放锁（由框架自动完成），
     * 因为看门狗不会主动释放锁，若一直不释放则锁会永远存在。
     */
    @RequiredArgsConstructor
