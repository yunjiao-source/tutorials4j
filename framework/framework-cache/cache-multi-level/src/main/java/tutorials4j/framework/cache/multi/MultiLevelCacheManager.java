 ...
 * <p>对于每个缓存名称，会尝试从两个底层管理器中分别获取对应的 {@link Cache} 实例，
 * 然后包装成 {@link MultiLevelCache} 返回。</p>
 * <p>注意：如果一个缓存名称只存在于其中一个管理器，则 {@link #getCache(String)} 会返回 {@code null}，
 * 这保证了多级缓存必须同时有本地和远程两端的配置。</p>
 *
 * @author Yun Jiao
...
public class MultiLevelCacheManager extends AbstractCacheManager {
     ...
     * <p>该方法会遍历本地和远程管理器缓存名称的并集，对每个名称调用 {@link #getCache(String)} 获取缓存实例。</p>
     *
    ...
    /**
     * 根据缓存名称获取对应的多级缓存实例。
     * <p>只有当本地和远程管理器中都存在该名称的缓存时，才会返回一个有效的 {@link MultiLevelCache}，
     * 否则返回 {@code null}。</p>
     *
     * @param name 缓存名称
