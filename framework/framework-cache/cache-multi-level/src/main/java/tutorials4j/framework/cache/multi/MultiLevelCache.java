 ...
 * <p>缓存操作遵循以下策略：</p>
 * <ul>
 *     <li><b>读取（get）</b>：先查本地缓存，命中则直接返回；未命中则查远程缓存，若远程命中则回填本地缓存后返回。</li>
 *     <li><b>写入（put）</b>：同时写入本地和远程缓存。</li>
 *     <li><b>删除（evict）</b>：同时删除本地和远程缓存。</li>
 *     <li><b>清空（clear）</b>：同时清空本地和远程缓存。</li>
 * </ul>
 *
 * @author Yun Jiao
 * @see Cache
...
public class MultiLevelCache implements Cache {
    ...
    /**
     * 根据键获取缓存值的包装器。
     * <p>查找顺序：本地缓存 → 远程缓存（并回填本地）→ 返回 null。</p>
     *
     * @param key 缓存键
     ...
     * <p>查找顺序：本地缓存 → 远程缓存（并回填本地）→ 若均不存在则调用 valueLoader 加载并写入两级缓存。</p>
     *
     * @param key         缓存键
