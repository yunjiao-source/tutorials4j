...
/**
 * 自定义的 {@link ModelResolver}，扩展 Swagger 默认模型解析器，
 * 为 Swagger 原生不处理的校验注解添加供应商扩展（{@code x-validation-*}）。
 *
 * <p>该解析器在生成 Schema 时处理 Bean Validation 注解。
 * 对于 Swagger 已原生处理的注解（如 {@code NotNull}、{@code Min}、{@code Size} 等），
 * 会跳过扩展添加逻辑。对于其他属于允许包（Jakarta Validation、Hibernate Validator
 * 以及 {@code LocalDateTimeFormat} 所在包）的校验注解，该解析器会提取其属性值，
 * 并将其作为扩展添加到对应的 Schema 属性上。
 *
 * <p>扩展键格式为 {@code x-validation-<注解简单名称>}，扩展值根据注解是否有属性而定：
 * <ul>
 *   <li>有属性时：返回属性名到属性值的映射（{@link LinkedHashMap}）</li>
 *   <li>无属性时：返回 {@code Boolean.TRUE}</li>
 * </ul>
 *
 * @author Yun Jiao
 * @see ModelResolver
...
public class DefaultSwaggerModelResolver extends ModelResolver {
     ...
     *
     * <p>包含 Jakarta Validation 约束、Hibernate Validator 约束
     * 以及自定义注解 {@code LocalDateTimeFormat} 所在的包。
     ...
     *
     * <p>如果注解没有声明任何方法（即没有属性），则返回 {@code Boolean.TRUE}。
     * 否则返回一个 {@link LinkedHashMap}，键为属性名，值为通过反射获取的属性值。
     *
