 ...
 *
 * <p>该过滤器从当前线程的 MDC 中提取需要传播的追踪键值对（如 traceId、spanId），将其添加到请求头中， 并将 MDC 快照存入 Reactor
 * 的上下文（Context）中，以保证在响应式操作链的任意线程上都能恢复上下文。 同时在收到响应时，从响应头中提取追踪信息并更新当前 MDC，支持下游服务生成新追踪标识后的回传。
 *
 * @author Yun Jiao
...
public class TraceExchangeFilterFunction implements ExchangeFilterFunction {
  ...
  public Mono<ClientResponse> filter(ClientRequest request, ExchangeFunction next) {
    // 从当前线程 MDC 获取快照
    Map<String, String> currentMdc =
        MDC.getCopyOfContextMap() != null ? MDC.getCopyOfContextMap() : Map.of();

    // 过滤出需要传播的键值对
    Set<String> mdcKeys = Set.of(DefaultConsts.HTTP_MDC_KEYS);
    Map<String, String> toPropagate =
    ...
    ClientRequest filteredRequest =
        ClientRequest.from(request).headers(headers -> toPropagate.forEach(headers::set)).build();

    if (log.isDebugEnabled()) {
      log.debug("跟踪信息，MethodName = {}, Url = {}", request.method().name(), request.url());
    }
